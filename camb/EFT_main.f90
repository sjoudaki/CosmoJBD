...
contains
	...
	real(dl) :: EFTbd_weff, EFTbd_weffprime !Dummy variables for BD dark energy equation of state and its derivative w.r.t. lna
	...
	if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
        EFTtemp_H0     = sqrt(Designed_EFT_Function(hubble, 1.0_dl, 0._dl)**2.0/(grhom/3.0))*100
        ...
        EFTtemp_grhom = 3.0*100.0**2.0/c_EFT**2*1000**2
        ...
        EFT_H0 = (EFTtemp_H0/c_EFT)*1000._dl
        ...
        grhov_t=Designed_EFT_Function(omegadebara2,a,0._dl)
	...
	if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then	
        ...
        gpres        = gpres_matter + Designed_EFT_function(omegadepresbara2,a,0._dl)
	else
        grho_matter  = grho_matter  +EFT_grhonu_tot
        gpres_matter = gpres_matter +EFT_gpinu_tot
        grho         = grho_matter  +grhov_t
	...
	!MODIFIED for JBD end
		...
		!MODIFIED for JBD start
	    else if (CP%DesignerEFTmodel==4 .or. CP%DesignerEFTmodel==5) then
                EFTc = EFTcTemp(a,0)
                EFTcdot = EFTcTemp(a,1)
                EFTLambda = EFTLambdaTemp(a,0)
                EFTLambdadot = EFTLambdaTemp(a,1)
