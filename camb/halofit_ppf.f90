    ...
    !HW, NAL, SJ Nov 20: version includes varying feedback parameters and JBD gravity corrections
    !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ! xxx JBD modifications xxx
    !===========================================
    integer, parameter :: halofit_jbd = 666
    !===========================================
  ...
  !!AM - End of my additions
    ...
    IF(halofit_version==halofit_mead .OR. halofit_version==halofit_halomodel .OR. halofit_version==halofit_jbd) THEN
    ...
    if (CP%EFTFLAG .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
      omm0 = (CP%omegac+CP%omegab+CP%omegan)/(CP%H0/100.0)**2.0
      fnu = (CP%omegan/(CP%H0/100.0)**2.0)/omm0
    else
    ...
    do itf = 1, CAMB_Pk%num_z
        ...
        if (CP%EFTFLAG .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
        om_m = omega_m(a, omm0, CP%omegav/(CP%H0/100.0)**2.0, CP%EFTw0,CP%EFTwa)
        om_v = omega_v(a, omm0, CP%omegav/(CP%H0/100.0)**2.0, CP%EFTw0,CP%EFTwa)  
      else
...
!Modified by AM
 ...
 !!AM Below is for HMcode
  SUBROUTINE HMcode(CAMB_Pk)
    ...
    ! xxx JBD modifications xxx
    !===========================================
    IF(halofit_version==halofit_jbd) imead=2

    !===========================================
    ! xxx JBD modifications xxx
    !===========================================
    ! If we comment out this line then it's normal HMCode
    ! with no JBD modifications active
    IF(halofit_version==halofit_mead) imead=2
    !===========================================
...
!!!    !$OMP PARALLEL DO FIRSTPRIVATE(linspec,cosi,PKSJ,zint,zshift,p, lut,jsj,karray,i,plin,p1h,p2h,pfull,pkd)
  ...
  FUNCTION Delta_v(z,lut,cosm)
    ...
    Real(dl) :: deltav0
!    Real(dl) :: wBDeff, d0, fac
    Real(dl) :: wBDeff, fac
    
    ! The fiducial value in HMCode
    deltav0 = 418.0d0
    ...
    if (CP%EFTFLAG .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
      ...
      d0  = 320.0 + 40.0 * z**0.26
      fac = atan( (abs(wBDeff - 50.0) * 0.001)**0.2)*2.0/acos(-1.0)
      
      ! Fitting formula based on simulations
      deltav0 = d0 + (deltav0 - d0) * fac
    endif
    !=============================================
    ...
    ELSE IF(imead==1 .or. imead==2) THEN
       ...
       Delta_v=deltav0*(Omega_m_hm(z,cosm)**(-0.352))
  ...
  FUNCTION delta_c(z,lut,cosm)
    ...
    ! xxx JBD modifications xxx
    !=============================================
    ! This should be commented out for consistency
    ! with wBD-> infty == standard HMCode. It only
    ! induces small <% level effects on large scales
    ! so just left it for hvaing consistency with previous runs
    IF(imead==2) THEN
      delta_c=delta_c*0.998
    END IF
    !=============================================
  ...
  FUNCTION eta(z,lut,cosm)
    ...
    ELSE IF(imead==1 .or. imead==2) THEN
       ...
       ! eta=0.603-0.3*lut%sig8z !original
       ! eta=cosm%barybloatcambhf-0.3*lut%sig8z !SJ, vary \eta_0 freely
       ! eta=(1.03-0.11*cosm%baryfeedcambhf)-0.3*lut%sig8z !SJ, vary \eta_0 as function of A
       eta=(0.98-0.12*cosm%baryfeedcambhf)-0.3*lut%sig8z !SJ, vary \eta_0 as function of A --- new version
       ! eta=0.6857-0.3*lut%sig8z !SJ, corresponding to 0.496 assuming \eta_0 - A relation
       !=============================================
    END IF
  ...
  FUNCTION As(z,lut,cosm)
    ...
    ELSE IF(imead==1 .or. imead==2) THEN
       ...
       ! The fiducial value is As=3.13
       As=cosm%baryfeedcambhf !SJ
       ! As=CP%baryfeed !SJ
       ! If this parameter is not set it will have the value 0.0
       ! so just ensure that in this case we use the fiducial value
       if(As > 0.0d0) then
          As=cosm%baryfeedcambhf
       else
          As=3.13d0
       endif
       !=============================================
  ...
  SUBROUTINE fill_plintab(iz,cosm,CAMB_PK)
    ...
    nk=CAMB_PK%num_k
    ALLOCATE(cosm%k_plin(nk),cosm%plin(nk),cosm%plinc(nk))
  ...
  FUNCTION Tcb_Tcbnu_ratio(k,z,cosm)
    ...
    ELSE
       ...
       !For my purpose (just the ratio) seems to work better using the EdS growth function result
       !D=(1.+zeq)*grow(z,cosm)
       D=(1.+zeq)/(1.+z)
       ...
       N_nu=3.
       yfs=17.2*cosm%f_nu*(1.+0.488*cosm%f_nu**(-7./6.))*(N_nu*q/cosm%f_nu)**2.
  ...
  SUBROUTINE assign_cosmology(cosm)
    ...
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
      cosm%om_m=(CP%omegac+CP%omegab+CP%omegan)/(CP%H0/100.0)**2.0
      cosm%om_v=CP%omegav/(CP%H0/100.0)**2.0
      cosm%f_nu=(CP%omegan/(CP%H0/100.0)**2.0)/cosm%om_m
      cosm%w=CP%EFTw0
      cosm%wa=CP%EFTwa
      cosm%h=CP%H0/100.
    else
    ...
    ! xxx JBD modifications xxx
    !===========================================
    cosm%baryfeedcambhf=CP%baryfeed 
    cosm%barybloatcambhf=CP%barybloat 
    !===========================================
  ...
  SUBROUTINE allocate_LUT(lut)
    ...
    !Allocates memory for the HMcode look-up tables
    IMPLICIT NONE
    TYPE(tables) :: lut
    INTEGER :: n
  ...
  SUBROUTINE deallocate_LUT(lut)
    ...
    !Deallocates HMcode look-up tables
    IMPLICIT NONE
    TYPE(tables) :: lut
  ...
  FUNCTION inttab(x,y,iorder)
    ...
    IF(n .NE. SIZE(y)) ERROR STOP 'Tables must be of the same length'
...
!    jmax=20
  ...
  FUNCTION Hubble2(z,cosm)
    ...
    REAL(dl) :: phi_aux, omde_aux, a_aux
    ...
    !Modified for EFT start
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
    phi_aux = Designed_EFT_Function(phibd,a_aux,0._dl)
    omde_aux = Designed_EFT_Function(omegade,a_aux,0._dl)
    Hubble2=(om_m*(1.+z)**3.)/phi_aux + omde_aux
    else
  ...
  FUNCTION Omega_m_hm(z,cosm)
    ...
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
      a_aux = 1.0/(1.0+z)
      phi_aux = Designed_EFT_Function(phibd,a_aux,0._dl)
      Omega_m_hm=(om_m*(1.+z)**3.)/(phi_aux*Hubble2(z,cosm))
    else
  ...
  FUNCTION derivative_table(x,xin,yin,iorder,imeth)
    ...
    n=SIZE(xin)
    IF(n .NE. SIZE(yin)) ERROR STOP 'FIND: Tables not of the same size'
  ...
  FUNCTION find(x,xin,yin,iorder,imeth)
    ...
    !Interpolation routine to get the value y(x) from tables of x and y
    IMPLICIT NONE
    REAL :: find
    REAL, INTENT(IN) :: x, xin(:), yin(:)
    REAL, ALLOCATABLE ::  xtab(:), ytab(:)
  ...
  FUNCTION AH(z,cosm)
    ...
    TYPE(cosmology), INTENT(IN) :: cosm
    !Modified for EFT start
    real(dl) :: phi_aux, weff_aux, omde_aux,a_aux
    !Modified for EFT end
    ...
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
      phi_aux = Designed_EFT_Function(phibd,a_aux,0._dl)
      omde_aux = Designed_EFT_Function(omegade,a_aux,0._dl)
      weff_aux = Designed_EFT_Function(omegadeweff_halofit,a_aux,0._dl)
      AH=cosm%om_m*(a**(-3.))+omde_aux*(1.+3.*weff_aux)
      AH=-AH/2.
    else
