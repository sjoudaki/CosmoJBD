            ...
            case (2) ! Designer mapping EFT
                select case (CP%DesignerEFTModel)
                    ...
                    case (2) ! Designer mc quintessence: c is obtained through EFT designer approach (in equations.f90).
                        EFTcTemp = 0._dl
    !Modified for JBD start
                    case (4) ! Standard Brans-Dicke model
                        if (deriv .eq. 0) then
                        EFTcTemp = Designed_EFT_Function(EFT_c_des, a, 0._dl)
                        else if (deriv .eq. 1) then
                        EFTcTemp = Designed_EFT_Function(EFT_cdot_des, a, 0._dl) 
                        end if
                    case (5) ! designer Brans-Dicke model
                        if (deriv .eq. 0) then
                        EFTcTemp = Designed_EFT_Function(EFT_c_des, a, 0._dl)
                        else if (deriv .eq. 1) then
                        EFTcTemp = Designed_EFT_Function(EFT_cdot_des, a, 0._dl) 
