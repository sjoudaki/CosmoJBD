...
!
!   Updated for JBD gravity by N. A. Lima in 2017
! -------------------------------------------------------------------------------------------------
...
module EFTdesigner
    ...
    integer , parameter :: des_nstep = 10000     ! Number of points sampled by the designer code.
    ...
    integer , parameter :: DesignerInFuture = 10    ! How many points in the future will the code sample
    ...
    real(dl), save, dimension(des_nstep+DesignerInFuture+1) :: omegadepresbara2, omegadeweff, omegadeweffprime, omegadeweff_halofit
...
contains
    ...
    subroutine EFTCAMB_Designer(success)
	    ...
	    case (5) !designer Brans-Dicke
		call EFTCAMB_designer_BD(success)
		!MODIFIED for JBD end
            case default
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!STANDARD BRANS-DICKE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    real(dl), parameter :: prec = 1.0e-12 !Precision for phi(a=1) = phi_0
    ...
    omr = omgamma + omnu
    aeq = omr/om
    arglamda = EFT_grhom*CP%omegav  !'weight' of the potential
  ...
  !  print*, 'The present-day value of Hubble is', H0_aux
    !print*, 'The flatness condition yieds', 1.0-(omegade(des_nstep) + om/(3.0*phibd(des_nstep)*(hubble(des_nstep))**2.0))
...
!!!!!ALPHA FUNCTIONS!!!!!
    ...
    do i = 1, des_nstep+DesignerInFuture
    omegade(i) = dedensitybd_halofit(a_evol(1+i),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    end do
...
real(dl) :: EFT_grhom, om, omr, omnumassive, omnu, omgamma, aeq
   ...
   omegadeweff_halofit(1) = omegapresde(1)/omegade(1)
   ...
   Omega3pDes(1) = Exp(-3.0*xaux)*(2.0*phiprimebd(1)-3.0*funcaobd(phibd(1),phiprimebd(1),EXP(xaux),om,omr,EFT_grhom,arglamda))
	...
	omegadeweff_halofit(1+i) = omegapresde(1+i)/omegade(1+i)
...
function Hubblebd(a,phi,phiprime,om,omr,H0,arglamda)
    ...
    integer  :: nu_i
        ...
        if (CP%Num_Nu_Massive /= 0) then                                            
            do nu_i = 1, CP%Nu_mass_eigenstates
                EFT_grhonu    = 0._dl
                EFT_gpinu     = 0._dl
                grhormass_t=grhormass(nu_i)/a**4
    ...
    Hubblebd = sqrt((Eom(a,om)/3.0 + Eomr(a,omr)/3.0 + potbd(phi,0,arglamda)/3.0 +&
    EFT_grhonu_tot/3.0)/(phi + phiprime - (CP%EFTwbds/phi)*phiprime**(2.0)/6.0))
...
function funcaobd(phi,phiprime,a,om,omr,H0,arglamda)
    ...
    integer  :: nu_i
        ...
        if (CP%Num_Nu_Massive /= 0) then                                            
            do nu_i = 1, CP%Nu_mass_eigenstates
                EFT_grhonu    = 0._dl
                EFT_gpinu     = 0._dl
                grhormass_t=grhormass(nu_i)/a**4.0
                call Nu_background(a*nu_masses(nu_i),EFT_grhonu,EFT_gpinu)
                EFT_grhonu_tot = EFT_grhonu_tot + grhormass_t*EFT_grhonu
                EFT_gpinu_tot  = EFT_gpinu_tot + grhormass_t*EFT_gpinu
            end do
    ...
    (Eom(a,om) + EFT_grhonu_tot - 3.0*EFT_gpinu_tot + 4.0*potbd(phi,0,arglamda) - 2.0*phi*potbd(phi,1,arglamda))/(hubble_aux**2.0*(3.0+2.0*CP%eftwbds))
...
function Hubbleprimereduced(a,phi,phiprime,om,omr,H0,arglamda) !i.e., not containing the phidprime term. This is actually H*H'
    ...
    Hubbleprimereduced = -0.5*(1.0+0.5*phiprime/phi)**(-1.0)*(3.0*hubble_aux**2.0 + 1.0/3.0*Eomr(a,omr)/phi + EFT_gpinu_tot/phi + 0.5*CP%eftwbds*hubble_aux**2.0*phiprime**2.0/phi**2.0 + 2.0*hubble_aux**2.0*phiprime/phi - potbd(phi,0,arglamda)/phi)
...
function phidotdot(phi,phiprime,a,om,omr,H0,arglamda) !in physical time
    ...
    phidotdot = -3.0*hubble_aux**2.0*phiprime + (Eom(a,om) + EFT_grhonu_tot - 3.0*EFT_gpinu_tot)/(3.0+2.0*CP%EFTwbds) + (4.0*potbd(phi,0,arglamda) - 2.0*phi*potbd(phi,1,arglamda))/(3.0+2.0*CP%EFTwbds) 
...
implicit none
    ...
    dedensitybara4 =  1.0/phi*(a*om + omr + EFT_grhonu_tot + a**4.0*0.5*CP%EFTwbds*phiprime**2.0*hubble_aux**2.0/phi - a**4.0*3.0*hubble_aux**2.0*phiprime + a**4.0*potbd(phi,0,arglamda)) - a*om - omr - EFT_grhonu_tot
...
implicit none
    ...
    dedensitybara2 =  1.0/phi*(om/a + omr/a**2.0 + EFT_grhonu_tot + a**2.0*0.5*CP%EFTwbds*phiprime**2.0*hubble_aux**2.0/phi - a**2.0*3.0*hubble_aux**2.0*phiprime + a**2.0*potbd(phi,0,arglamda)) - om/a - omr/a**2.0 - EFT_grhonu_tot
...
function depresbara4(a,phi,phiprime,om,omr,H0,arglamda)
    ...
    depresbara4 = 1.0/phi*(1.0/3.0*omr + EFT_gpinu_tot + a**4.0*0.5*CP%EFTwbds*phiprime**2.0*hubble_aux**2.0/phi + a**4.0*2.0*hubble_aux**2.0*phiprime + a**4.0*phidotdot(phi,phiprime,a,om,omr,H0,arglamda) - a**4.0*potbd(phi,0,arglamda)) - 1.0/3.0*omr - EFT_gpinu_tot
...
function depresbara2(a,phi,phiprime,om,omr,H0,arglamda)
    ...
    depresbara2 = 1.0/phi*(1.0/3.0*omr/a**2.0 + EFT_gpinu_tot + a**2.0*0.5*CP%EFTwbds*phiprime**2.0*hubble_aux**2.0/phi + a**2.0*2.0*hubble_aux**2.0*phiprime + a**2.0*phidotdot(phi,phiprime,a,om,omr,H0,arglamda) - a**2.0*potbd(phi,0,arglamda)) - 1.0/3.0*omr/a**2.0 - EFT_gpinu_tot


end function depresbara2

function Hdotbd(a,phi,phiprime,om,omr,H0,arglamda) !w.r.t. to physical time
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,Hdotbd,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    Hdotbd = 0.5*(-3.0*hubble_aux**2.0 - 1.0/3.0*Eomr(a,omr)/phi - EFT_gpinu_tot/phi -0.5*CP%EFTwbds*hubble_aux**2.0*phiprime**2.0/phi**2.0 - 2.0*hubble_aux**2.0*phiprime/phi - phidotdot(phi,phiprime,a,om,omr,H0,arglamda)/phi + potbd(phi,0,arglamda)/phi)
...
function dedensitybd(a,phi,phiprime,om,omr,H0,arglamda) !Returns fractional energy density (/3.0*H**2.0*phi), used in halofit_ppf
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,dedensitybd,arglamda

    hubble_aux = Hubblebd(a,phi,phiprime,om,omr,H0,arglamda)

    dedensitybd =  0.5*CP%EFTwbds*phiprime**2.0/(3.0*phi**2.0) - phiprime/phi + potbd(phi,0,arglamda)/(3.0*phi*hubble_aux**2.0)

end function dedensitybd

function dedensitybd_halofit(a,phi,phiprime,om,omr,H0,arglamda) !Returns fractional energy density (/3.0*H**2.0*phi), used in halofit_ppf
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,hubble_today,dedensitybd_halofit,arglamda

    hubble_aux = Hubblebd(a,phi,phiprime,om,omr,H0,arglamda)
    hubble_today = Hubblebd(1.0_dl,phi,phiprime,om,omr,H0,arglamda)

    dedensitybd_halofit =  0.5*CP%EFTwbds*hubble_aux**2.0*phiprime**2.0/(3.0*hubble_today**2.0*phi**2.0) - phiprime*hubble_aux**2.0/(hubble_today**2.0*phi) + potbd(phi,0,arglamda)/(3.0*phi*hubble_today**2.0)

end function dedensitybd_halofit
...
function depresbd(a,phi,phiprime,om,omr,H0,arglamda)
    ...
    depresbd = 0.5*CP%EFTwbds*phiprime**2.0/(3.0*phi**2.0) + 2.0*phiprime/(3.0*phi) + &
	phidotdot(phi,phiprime,a,om,omr,H0,arglamda)/(3.0*phi*hubble_aux**2.0) - potbd(phi,0,arglamda)/(3.0*phi*hubble_aux**2.0)
...
!Modified for JBD start
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!DESIGNER BRANS-DICKE MODEL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


    subroutine EFTCAMB_designer_BD(success)
    !This subroutine solves the equation of motion for the designer Brans-Dicke model
    implicit none
    logical :: success
    logical :: Ok
    integer :: i, try
    real(dl) :: h, xaux,k1,k2,F1
    real(dl) :: om, omr, omnumassive, omnu, omgamma, aeq, arglamda, EFT_grhom, H0_aux
    real(dl) :: phi_i, phiprime_i, phibdi_1, phibdi_2, hubble_1, hubble_2
    real(dl) :: delta_phi, phibdi_final, hubble_final, phibdf_1, phibdf_2, phibdf_final, phibdi_aux
    real(dl), parameter :: haux = 1.0e-6
    real(dl), parameter :: prec = 1.0e-12
    real(dl), parameter :: wbdmax = 1.0e10
    ...
    aeq = omr/om
    arglamda = EFT_grhom*CP%omegav !'weight' of the potential
    phibdi_1 = CP%EFTphi_0*aeq**(1.0/(1.0+CP%EFTwbd)) - 0.2 !starting guess point for the scalar field
    phibdi_2 = CP%EFTphi_0*aeq**(1.0/(1.0+CP%EFTwbd)) + 0.2


   100 call evolvebackgrounddesigner(phibdi_1,phibdf_1,arglamda)
 
    if (abs(phibdf_1 - CP%EFTphi_0) > prec) then  
    Ok = .false.
   
   call evolvebackgrounddesigner(phibdi_2,phibdf_2,arglamda)

        if(phibdf_1 > CP%EFTphi_0 .or. phibdf_2 < CP%EFTphi_0) then
		!if (CP%EFTwbd < wbdmax) then
		print*, 'You have to be within the range to determine the appropriate initial conditions'
		print*, 'Please check the range of your initial conditions for the scalar field'
		!stop
		phibdi_aux = phibdi_1
    ...
    do i=1,1000
		...
		call EvolveBackgrounddesigner(phibdi_final,phibdf_final,arglamda)
    ...
    print*, 'The final ratio of phi/phi_0 is', phibdf_final/CP%EFTphi_0
    H0_aux = sqrt(hubble(des_nstep)**2.0/(EFT_grhom/3.0))*100.0
  !  print*, 'The present-day hubble is', H0_aux
    print*, 'The flatness condition yieds', 1.0-(omegade(des_nstep) + om/(3.0*phibd(des_nstep)*(hubble(des_nstep))**2.0))
    if (CP%EFTBDBack_debug .eq. 1) then
    open(100,file='a_evol.txt',status='replace')
    open(200,file='phi_a.txt',status='replace')
    open(300,file='phiprime_a.txt',status='replace')
    open(400,file='hubble.txt',status='replace')
    open(500,file='weff.txt',status='replace')
    open(600,file='omegade.txt',status='replace')
    ...
    write(200,*), phibd(1)
    write(300,*), phiprimebd(1)
    write(400,*), sqrt(Ebd(a_evol(1),om,omr,phibd(1),arglamda))/sqrt(EFT_grhom/3.0)*100.0
    write(500,*), omegadeweff(1)
    write(600,*), omegade(1)
    do i = 1, des_nstep+DesignerInFuture
    write(200,*), phibd(1+i)
    write(100,*), a_evol(1+i)
    write(300,*), phiprimebd(1+i)
    write(400,*), sqrt(Ebd(a_evol(1+i),om,omr,phibd(1+i),arglamda))/sqrt(EFT_grhom/3.0)*100.0
    write(500,*), omegadeweff(i+1)
    write(600,*), omegade(i+1)
    end do
    end if 

    return
    end subroutine EFTCAMB_designer_BD


    subroutine evolvebackgrounddesigner(phi_i,phibd_aux,arglamda)
    implicit none
    real(dl), intent(in) :: phi_i,arglamda
    real(dl), intent(out) :: phibd_aux
    integer :: i,aux
    real(dl) :: xaux, h, k1,k2,k3,k4,F1,F2,F3
    real(dl) :: H0_EFT, om, omr, omnumassive, omnu, omgamma, aeq, EFT_grhom
    ...
    phibd(1) = phi_i
    phiprimebd(1) = 1.0e-10_dl !In radiation domination, attractor solution is phi = k, and any initial velocity is quickly damped
    hubble(1) = Hubblebd_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    hubble_lam(1) = sqrt(om*a_evol(1)**(-3.0)/3.0 + omr*a_evol(1)**(-4.0)/3.0 + arglamda/3.0)
    omegade(1) = dedensitybd_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    omegadebara4(1) = dedensitybara4_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    omegadebara2(1) = dedensitybara2_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    omegadepresbara4(1) = depresbara4_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    omegadepresbara2(1) = depresbara2_des(exp(xaux),phibd(1),phiprimebd(1),om,omr,EFT_grhom,arglamda)
    omegadeweff(1) = omegadepresbara4(1)/omegadebara4(1)
    !!!!!EFT QUANTITIES!!!!!!
    OmegaDes(1) = phibd(1) - 1.0_dl
    OmegapDes(1) = EXP(-xaux)*phiprimebd(1)
    OmegappDes(1) = EXP(-2.0*xaux)*(funcao(phibd(1),phiprimebd(1),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda) - phiprimebd(1))
    Omega3pDes(1) = Exp(-3.0*xaux)*(2.0*phiprimebd(1)-3.0*funcao(phibd(1),phiprimebd(1),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)) !this is an approximation that neglects phi^{prime prime prime}

    EFT_Lambda_Des(1) = 0.5_dl*EXP(2.0*xaux)*(Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*CP%EFTwbd/phibd(1))*phiprimebd(1)**2.0 - EXP(2.0*xaux)*pot(EXP(xaux),om,omr,phibd(1),arglamda)

    EFT_Lambdadot_Des(1) = EXP(3.0*xaux)*sqrt(Ebd(EXP(xaux),om,omr,phibd(1),arglamda))*(0.5*CP%EFTwbd*(2.0*HprimeHbd(EXP(xaux),om,omr,phibd(1),phiprimebd(1),arglamda)*phiprimebd(1)**2.0/phibd(1) + 2.0*Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*funcao(phibd(1),phiprimebd(1),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)*phiprimebd(1)/phibd(1) - Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*phiprimebd(1)**3.0/phibd(1)**2.0) - potdphi(EXP(xaux),om,omr,phibd(1),phiprimebd(1),arglamda)*phiprimebd(1))

    EFT_c_Des(1) = 0.5_dl*EXP(2.0*xaux)*(Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*CP%EFTwbd/phibd(1))*phiprimebd(1)**2.0
  
    EFT_cdot_Des(1) = EXP(3.0*xaux)*sqrt(Ebd(Exp(xaux),om,omr,phibd(1),arglamda))*(0.5*CP%EFTwbd*(2.0*HprimeHbd(EXP(xaux),om,omr,phibd(1),phiprimebd(1),arglamda)*phiprimebd(1)**2.0/phibd(1) + 2.0*Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*funcao(phibd(1),phiprimebd(1),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)*phiprimebd(1)/phibd(1) - Ebd(EXP(xaux),om,omr,phibd(1),arglamda)*phiprimebd(1)**3.0/phibd(1)**2.0))
    !!!!!EFT QUANTITIES!!!!!!

    do i = 1, des_nstep+DesignerInFuture
    k1 = funcao(phibd(i),phiprimebd(i),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)
    F1 = phiprimebd(i)
    
    k2 = funcao(phibd(i) + 0.5*h*F1,phiprimebd(i)+0.5*h*k1,EXP(xaux+0.5*h),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)
    F2 = phiprimebd(i) + 0.5*h*k1

    k3 = funcao(phibd(i) + 0.5*h*F2,phiprimebd(i)+0.5*h*k2,EXP(xaux+0.5*h),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)
    F3 = phiprimebd(i) + 0.5*h*k2

    k4 = funcao(phibd(i) + h*F3,phiprimebd(i)+h*k3,EXP(xaux+h),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)
    
    phibd(i+1) = phibd(i) + h*phiprimebd(i) + (1.0_dl/6.0_dl)*h**(2.0)*(k1+k2+k3)
    phiprimebd(i+1) = phiprimebd(i) + (1.0_dl/6.0_dl)*h*(k1+2.0*k2+2.0*k3+k4)

    xaux = xaux + h
    hubble(1+i) = Hubblebd_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    hubble_lam(1+i) = sqrt(om*a_evol(1+i)**(-3.0)/3.0 + omr*a_evol(1+i)**(-4.0)/3.0 + arglamda/3.0)
    omegade(1+i) = dedensitybd_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    omegadebara4(1+i) = dedensitybara4_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    omegadebara2(1+i) = dedensitybara2_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    omegadepresbara4(1+i) = depresbara4_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    omegadepresbara2(1+i) = depresbara2_des(exp(xaux),phibd(1+i),phiprimebd(1+i),om,omr,EFT_grhom,arglamda)
    omegadeweff(1+i) = omegadepresbara4(1+i)/omegadebara4(1+i)
    omegadeweffprime(i) = (omegadeweff(i+1) - omegadeweff(i))/h
    if (i .eq. des_nstep + DesignerInFuture) then
    omegadeweffprime(i+1) = omegadeweffprime(i)
    end if
    !!!!!EFT QUANTITIES!!!!!
    OmegaDes(1+i) = phibd(i+1) - 1.0_dl
    OmegapDes(1+i) = EXP(-xaux)*phiprimebd(1+i)
    OmegappDes(1+i) = EXP(-2.0*xaux)*(funcao(phibd(1+i),phiprimebd(1+i),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)-phiprimebd(i+1))
    Omega3pDes(1+i) = Exp(-3.0*xaux)*(2.0*phiprimebd(1+i)-3.0*funcao(phibd(1+i),phiprimebd(1+i),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)) !this is an approximation that neglects phi^{prime prime prime} as this should be of order (1+wbd)**-3.0 in matter domination

    EFT_Lambda_Des(1+i) = 0.5_dl*EXP(xaux*2.0)*(Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*CP%EFTwbd/phibd(1+i))*phiprimebd(1+i)**2.0 - EXP(xaux*2.0)*pot(EXP(xaux),om,omr,phibd(1+i),arglamda)

    EFT_Lambdadot_Des(1+i) = EXP(3.0*xaux)*sqrt(Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda))*(0.5*CP%EFTwbd*(2.0*HprimeHbd(EXP(xaux),om,omr,phibd(1+i),phiprimebd(1+i),arglamda)*phiprimebd(1+i)**2.0/phibd(1+i) + 2.0*Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*funcao(phibd(1+i),phiprimebd(1+i),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)*phiprimebd(1+i)/phibd(1+i) - Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*phiprimebd(1+i)**3.0/phibd(1+i)**2.0) - potdphi(EXP(xaux),om,omr,phibd(1+i),phiprimebd(1+i),arglamda)*phiprimebd(1+i))

    EFT_c_Des(1+i) = 0.5_dl*EXP(xaux*2.0)*(Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*CP%EFTwbd/phibd(1+i))*phiprimebd(1+i)**2.0

    EFT_cdot_Des(1+i) = EXP(3.0*xaux)*sqrt(Ebd(Exp(xaux),om,omr,phibd(1+i),arglamda))*(0.5*CP%EFTwbd*(2.0*HprimeHbd(EXP(xaux),om,omr,phibd(1+i),phiprimebd(1+i),arglamda)*phiprimebd(1+i)**2.0/phibd(1+i) + 2.0*Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*funcao(phibd(1+i),phiprimebd(1+i),EXP(xaux),om,omr,CP%EFTwbd,CP%EFTw0,CP%h0,arglamda)*phiprimebd(1+i)/phibd(1+i) - Ebd(EXP(xaux),om,omr,phibd(1+i),arglamda)*phiprimebd(1+i)**3.0/phibd(1+i)**2.0))
    !!!!!EFT QUANTITIES!!!!!
    ...
    phibd_aux = phibd(des_nstep)!+DesignerInFuture)

    end subroutine evolvebackgrounddesigner


    function Eeff(a,om,omr,weff,arglamda) !This function returns the effective dark energy contribution to the designer BD model
    implicit none
    real(dl), intent(in) :: a,om,omr,weff,arglamda
    real(dl) :: Eeff
    
    Eeff = arglamda*a**(-3*(1+weff))

    end function Eeff

function Hubblebd_des(a,phi,phiprime,om,omr,H0,arglamda)
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,Hubblebd_des,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    Hubblebd_des = sqrt((Eom(a,om)/3.0 + Eomr(a,omr)/3.0 + Eeff(a,om,omr,CP%EFTw0,arglamda)/3.0 + EFT_grhonu_tot/3.0)/(phi + phiprime - (CP%EFTwbds/phi)*phiprime**(2.0)/6.0))

end function  Hubblebd_des


    function Ebd(a,om,omr,phi,arglamda) !This function returns the hubble parameter for the designer BD model used in the backgroun calc.
    implicit none
    real(dl), intent(in) :: a, om, omr, phi,arglamda
    real(dl) :: Ebd
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    Ebd = (1._dl/phi)*(Eom(a,om)/3.0 + Eomr(a,omr)/3.0 + Eeff(a,om,omr,CP%EFTw0,arglamda)/3.0 + EFT_grhonu_tot/3.0)

    end function Ebd


    function Eprimebd(a,om,omr,phi,phiprime,arglamda) !E^{prime}
    implicit none
    real(dl), intent(in) :: a, om, omr, phi, phiprime, arglamda
    real(dl) :: Eprimebd
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t, EFT_EP_nu
    integer  :: nu_i
        ! Massive neutrinos
        EFT_grhonu_tot = 0._dl
        EFT_gpinu_tot = 0._dl
	EFT_EP_nu = 0._dl
        if (CP%Num_Nu_Massive /= 0) then                                            
            do nu_i = 1, CP%Nu_mass_eigenstates
                ...
                EFT_grhonu_tot = EFT_grhonu_tot + grhormass_t*EFT_grhonu
                EFT_gpinu_tot  = EFT_gpinu_tot + grhormass_t*EFT_gpinu
		EFT_EP_nu  = EFT_EP_nu - grhormass(nu_i)/a**4*(EFT_grhonu + EFT_gpinu)
            end do
        end if



    Eprimebd = 1._dl/phi*(-3.0*Eom(a,om)/3.0 - 4.0*Eomr(a,omr)/3.0 - 3.0*(1.0 + CP%EFTw0)*Eeff(a,om,omr,CP%EFTw0,arglamda)/3.0) - phiprime/phi*Ebd(a,om,omr,phi,arglamda)

    end function Eprimebd 

 
    function HprimeHbd(a,om,omr,phi,phiprime,arglamda)
    implicit none
    real(dl), intent(in) :: a, om, omr, phi, phiprime, arglamda
    real(dl) :: HprimeHbd

    HprimeHbd = 0.5*Eprimebd(a,om,omr,phi,phiprime,arglamda)

    end function HprimeHbd

    function pot(a,om,omr,phi,arglamda)
    implicit none 
    real(dl), intent(in) :: a, om, omr, phi, arglamda
    real(dl) :: pot
    
    pot = Eeff(a,om,omr,CP%EFTw0,arglamda)

    end function pot


    function potdphi(a,om,omr,phi,phiprime,arglamda)
    implicit none
    real(dl), intent(in) :: a, om, omr, phi, phiprime, arglamda
    real(dl) :: potdphi, phiprimeaux
    if (phiprime .eq. 0) then
    phiprimeaux = 1.0e-10
    else
    phiprimeaux = phiprime
    end if
    potdphi = -3.0*(1+CP%EFTw0)*Eeff(a,om,omr,CP%EFTw0,arglamda)/phiprimeaux
    end function potdphi


    function funcao(phi,phiprime,a,om,omr,wbd,w0,H0,arglamda) !phi^{prime prime}
    implicit none
    real(dl), intent(in) :: a, om, omr, phi, phiprime, wbd, w0, H0, arglamda
    real(dl) :: funcao

    funcao = -phiprime*(3.0 + 0.5*Eprimebd(a,om,omr,phi,phiprime,arglamda)/Ebd(a,om,omr,phi,arglamda)) + (Eom(a,om) + 4.0*pot(a,om,omr,phi,arglamda) - 2.0*phi*potdphi(a,om,omr,phi,phiprime,arglamda))/(Ebd(a,om,omr,phi,arglamda)*(2.0*wbd+3.0))

    end function funcao


function dedensitybd_des(a,phi,phiprime,om,omr,H0,arglamda) !Returns fractional energy density (/3.0*H**2.0*phi)
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,dedensitybd_des,arglamda

    hubble_aux = Hubblebd_des(a,phi,phiprime,om,omr,H0,arglamda)

    dedensitybd_des =  0.5*CP%EFTwbds*phiprime**2.0/(3.0*phi**2.0) - phiprime/phi + potbd(phi,0,arglamda)/(3.0*phi*hubble_aux**2.0)

end function dedensitybd_des


function dedensitybara4_des(a,phi,phiprime,om,omr,H0,arglamda)

implicit none

    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,dedensitybara4_des,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    dedensitybara4_des =  1.0/phi*(a*om + omr + EFT_grhonu_tot + a**4.0*0.5*CP%EFTwbd*phiprime**2.0*hubble_aux**2.0/phi - a**4.0*3.0*hubble_aux**2.0*phiprime + a**4.0*pot(a,om,omr,phi,arglamda)) - a*om - omr - EFT_grhonu_tot

end function dedensitybara4_des


function dedensitybara2_des(a,phi,phiprime,om,omr,H0,arglamda)

implicit none

    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,dedensitybara2_des,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    dedensitybara2_des =  1.0/phi*(om/a + omr/a**2.0 + EFT_grhonu_tot + a**2.0*0.5*CP%EFTwbd*phiprime**2.0*hubble_aux**2.0/phi - a**2.0*3.0*hubble_aux**2.0*phiprime + a**2.0*pot(a,om,omr,phi,arglamda)) - om/a - omr/a**2.0 - EFT_grhonu_tot

end function dedensitybara2_des


function depresbara4_des(a,phi,phiprime,om,omr,H0,arglamda)
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,depresbara4_des,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    depresbara4_des = 1.0/phi*(1.0/3.0*omr + EFT_gpinu_tot + a**4.0*0.5*CP%EFTwbd*phiprime**2.0*hubble_aux**2.0/phi + a**4.0*2.0*hubble_aux**2.0*phiprime + a**4.0*phidotdot_des(phi,phiprime,a,om,omr,H0,arglamda) - a**4.0*pot(a,om,omr,phi,arglamda)) - 1.0/3.0*omr - EFT_gpinu_tot

end function depresbara4_des


function depresbara2_des(a,phi,phiprime,om,omr,H0,arglamda)
    implicit none
    real(dl) :: a,phi,phiprime,om,omr,H0,hubble_aux,depresbara2_des,arglamda
    real(dl) :: EFT_grhonu_tot, EFT_gpinu_tot, EFT_grhonu, EFT_gpinu, grhormass_t
    integer  :: nu_i
    ...
    depresbara2_des = 1.0/phi*(1.0/3.0*omr/a**2.0 + EFT_gpinu_tot + a**2.0*0.5*CP%EFTwbd*phiprime**2.0*hubble_aux**2.0/phi + a**2.0*2.0*hubble_aux**2.0*phiprime + a**2.0*phidotdot_des(phi,phiprime,a,om,omr,H0,arglamda) - a**2.0*pot(a,om,omr,phi,arglamda)) - 1.0/3.0*omr/a**2.0 - EFT_gpinu_tot

end function depresbara2_des


function phidotdot_des(phi,phiprime,a,om,omr,H0,arglamda) !in physical time
    implicit none
    real(dl) :: phi, phiprime, a, om, omr, H0, hubble_aux, phidotdot_des, arglamda

    hubble_aux = Hubblebd_des(a,phi,phiprime,om,omr,H0,arglamda)

    phidotdot_des = -3.0*hubble_aux**2.0*phiprime + Eom(a,om)/(3.0+2.0*CP%EFTwbd) + (4.0*pot(a,om,omr,phi,arglamda) - 2.0*phi*potdphi(a,om,omr,phi,phiprime,arglamda))/(3.0+2.0*CP%EFTwbd) 

end function phidotdot_des
