	...
	real(dl) :: EFTwbds !standard Brans-dicke parameter
	real(dl) :: EFTwbd !designer Brans-Dicke parameter
        real(dl) :: EFTphi_0 !Brans-Dicke scalar field present-day value
	integer :: EFTpot, EFTBDback_debug !standard Brans-Dicke potential choice
        real(dl) :: EFTGeffG !present-day ratio of effective graviational coupling to G
    ...
    !Modified for JBD start
    subroutine CAMBParams_Set(P, Haux, phi_aux ,error, DoReion)
    !Modified for JBD end
    use constants
    type(CAMBparams), intent(in) :: P
    !Modified for JBD start
    real(dl), intent(out) :: Haux, phi_aux
    !Modified for JBD end
...
!Modified for JBD end
    ...
    grhom = 3*100**2.0/c**2*1000**2 !3*h0^2/c^2 (=8*pi*G*rho_crit/c^2)
    else
    grhom = 3*CP%h0**2.0/c**2*1000**2 !3*h0^2/c^2 (=8*pi*G*rho_crit/c^2)
    end if
    !MODIFIED for JBD end
	...
	!MODIFIED for JBD start
            call init_background(Haux,phi_aux)
		    ...
		    aux = phi_aux
		    phi_aux = phi_early
		    !adotrad = sqrt((grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates)))/(3*aux))
		    end if
	...
	if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTModel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
        ...
        write(*,'("Om_Lambda h^2        = ",f9.6)') CP%omegav
        write(*,'("Om_Lambda            = ",f9.6)') CP%omegav/(CP%H0/100)**2
	write(*,'("Om_m			= ",f9.6)') (CP%omegac + CP%omegab)/(CP%h0/100)**2
...
!              write(*,'("Om_m (1-Om_K-Om_L)   = ",f9.6)') 1-CP%omegak-CP%omegav
    ...
    !Modified for JBD start
    if (CP%EFTflag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
    TimeOfz=DeltaTime(1.0e-8_dl,1._dl/(z+1._dl))
    else
    ...
    !Modified for JBD start
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
    R=3.0d4*a*CP%omegab!*(CP%h0/100.0d0)**2 CP%Omegab already includes h**2 factor
    else
    ...
    function get_H0_from_background()
    real(dl) get_H0_from_background
    real(dl) get_H0_bd
    external get_H0_bd

    get_H0_from_background = get_H0_bd()

    end function get_H0_from_background
...
500     if (L< lmax_extrap_highl) &
    ...
    Transfer_growth = 14
    !Modified for JBD end
    integer, parameter :: Transfer_max = Transfer_growth!Transfer_vel_baryon_cdm
    ...
    !Modified for JBD start
    if (CP%EFTFlag .eq. 2 .and. (CP%DesignerEFTmodel .eq. 4 .or. CP%DesignerEFTmodel .eq. 5)) then
    call Recombination_Init(CP%Recomb, CP%omegac/(CP%H0/100.0)**2.0, CP%omegab/(CP%H0/100.0)**2.0,CP%Omegan/(CP%H0/100.0)**2.0, &
    CP%Omegav/(CP%H0/100.0)**2.0,CP%h0,CP%tcmb,CP%yhe,CP%Num_Nu_massless + CP%Num_Nu_massive,CP%EFTFlag,CP%DesignerEFTmodel)
    else
    call Recombination_Init(CP%Recomb, CP%omegac, CP%omegab,CP%Omegan, CP%Omegav, &
    CP%h0,CP%tcmb,CP%yhe,CP%Num_Nu_massless + CP%Num_Nu_massive,CP%EFTFlag,CP%DesignerEFTmodel)
    end if
    !Modified for JBD end
