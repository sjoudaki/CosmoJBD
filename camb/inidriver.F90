...
! EFTCAMB MOD START
    ...
    ! 4) Initialization of EFTCAMB model parameters.
	 ...
	 !Modified by Nelson!
	 ! read standard and designer Brans-Dicke parameters:
         P%EFTwbds = Ini_Read_Double('EFTwbds',0.0_dl)
	 P%EFTwbd = Ini_Read_Double('EFTwbd',0.0_dl)
	 P%EFTpot = Ini_Read_Int('EFTpot',0)
         P%EFTGeffG = Ini_Read_Double('EFTGeffG',0.0_dl)
	 P%EFTBDback_debug = Ini_Read_Int('BDback_debug',0)

         !Define Brans-Dicke present-day value
         if (P%EFTFlag .eq. 2 .and. (P%DesignerEFTmodel .eq. 4 .or. P%DesignerEFTmodel .eq. 5)) then
         if (P%DesignerEFTmodel .eq. 4) then
         P%EFTphi_0 = (P%EFTGeffG*(2.0*P%EFTwbds + 3.0)/(2.0*P%EFTwbds + 4.0))**(-1.0)
	 else if (P%DesignerEFTmodel .eq. 5) then
         P%EFTphi_0 = (P%EFTGeffG*(2.0*P%EFTwbd + 3.0)/(2.0*P%EFTwbd + 4.0))**(-1.0)
	 end if
         P%omegab = Ini_Read_Double('ombh2')!/(P%H0/100)**2
         P%omegac = Ini_Read_Double('omch2')!/(P%H0/100)**2
         P%omegan = Ini_Read_Double('omnuh2')!/(P%H0/100)**2
         P%omegav = Ini_Read_Double('omvh2')!1- Ini_Read_Double('omk') - P%omegab- P%omegac - P%omegan
         end if
	 !Modified by Nelson!
