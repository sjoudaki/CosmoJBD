    ...
    WantLateTime =  CP%DoLensing .or. num_redshiftwindows > 0
        ...
        ! EFTCAMB MOD START
         if ((EFTCAMBuseinCOSMOMC==1 .and. CP%EFTflag/=0 .and. CP%DesignerEFTmodel==1) .or. (EFTCAMBuseinCOSMOMC==1 .and. CP%EFTflag/=0 .and. CP%DesignerEFTmodel==2)) then
            call EFTCAMB_initialization(EFTsuccess)
            if (.not.EFTsuccess) stop
	!Modified for JBD start
        else if (EFTCAMBuseinCOSMOMC==1 .and. CP%EFTflag/=0 .and. (CP%DesignerEFTmodel==4 .or. CP%DesignerEFTmodel==5)) then
            !I HAVE ALREADY CALLED IT IN EQUATIONS_EFT.F90
            EFTsuccess = .true.
            if (.not.EFTsuccess) stop 
