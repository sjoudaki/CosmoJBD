...
!    Atomic data for HeI 
       ...
       contains
        ...
        subroutine Recombination_init(Recomb, OmegaC, OmegaB, Omegan, Omegav, h0inp,tcmb,yp, nnu, eftflag, eftmodelflag)
	...
	!Modified for EFT start
	integer, intent(in) :: eftflag, eftmodelflag
	!Modified for EFT end
...
!       will output every 1 in z, but this is easily changed also
 ...
 !Not general, but only for approx 
        ...
        !Modified for EFT start
 	if (eftflag .eq. 2 .and. (eftmodelflag .eq. 4 .or. eftmodelflag .eq. 5)) then
	OmegaK = 0.0_dl
        else
