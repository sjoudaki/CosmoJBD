    ...
    do i= 1, DataSets%Count
        ...
        CosmoSettings%use_growth_kz = .true. !SJ
    ...
    if (Ini%HasKey('zeff')) then
        ...
        do i=1,this%num_bao
            if(this%name=='BAORSDDR12') then
                read (F%unit,*, iostat=iopb) this%bao_z(i),bao_measurement,this%bao_obs(i)
             else
                read (F%unit,*, iostat=iopb) this%bao_z(i),this%bao_obs(i),this%bao_err(i)
             end if
            if (iopb /= 0) then
               write(*,*) this%tag,this%bao_z(i),this%bao_obs(i),i
               call MpiStop('BAO_ReadIni: Error reading bao_measurements_file: ' &
                //trim(this%name))
             end if
        end do
        call F%Close()
    ...
    omegam = 1.d0 - CMB%omv/(CMB%H0/100.0)**2.0 - CMB%omk
    else
...
!            BAO_theory(j) = Theory%growth_z%Value(z)
            BAO_theory(j) = Theory%growth_k_z%PowerAt(0.1d0,z)*Theory%sigma8_z%Value(z)!SJ
            case default
    ...
    if(feedback>3) then
       do j=1,this%num_bao
          write(*,*) this%bao_z(j),this%type_bao(j), BAO_theory(j), this%bao_obs(j)
       end do
       !write(*,*) 'invcov:', this%bao_invcov
       write(*,*) 'params: ',CMB%omdm, CMB%h0 , CMB%omb
    endif
