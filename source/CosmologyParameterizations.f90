    ...
    call this%SetTheoryParameterNumbers(54,last_power_index)
    ...
    class is (TCosmologyCalculator)
        ...
        class is (CMBParams)
            ...
            !Modified for EFT start
            if (Cosmosettings%EFTFlag .eq. 2 .and. (CosmoSettings%DesignerEFTmodel .eq. 4 .or. CosmoSettings%DesignerEFTmodel .eq.5)) then
            
            H0 = CosmoCalc%CMBH0BD()

            call SetForH(Params,CMB,H0,.false.)
            D_b = CosmoCalc%CMBToTheta(CMB) !Call again just to make sure everything is properly initialized
...
if (Cosmosettings%EFTFlag .eq. 2 .and. (CosmoSettings%DesignerEFTmodel .eq. 4 .or. CosmoSettings%DesignerEFTmodel .eq.5)) then
        derived(1) = CMB%H0
        derived(2) = CMB%omv/(CMB%H0/100.0)**2.0
        derived(3) = (CMB%omdm+CMB%omb)/(CMB%H0/100.0)**2.0
        derived(4) = CMB%omdmh2 + CMB%ombh2
        derived(5) = CMB%omnuh2
        derived(6) = (CMB%omdmh2 + CMB%ombh2)*CMB%h

        derived(7) = Theory%Sigma_8
        derived(8) = Theory%Sigma_8*((CMB%omdm+CMB%omb)/(CMB%H0/100.0)**2.0)**0.5_mcp
        derived(9) = Theory%Sigma_8*((CMB%omdm+CMB%omb)/(CMB%H0/100.0)**2.0)**0.25_mcp
        derived(10)= Theory%Sigma_8/CMB%h**0.5_mcp

        derived(11) = Theory%Lensing_rms_deflect
        derived(12) = CMB%zre
        derived(13) = LOG10(CMB%EFTwbds)
        derived(14) = CMB%EFTwbds
        ix=15
...
!Modified for EFT end
    ...
    if (firsttime) then
        ...
        CMB%fdm = Params(16)
        !Modified for EFT start
        CMB%baryfeed = Params(53)
        CMB%barybloat = Params(54)
        !Modified for EFT end
        call SetFast(Params,CMB)
	...
	!Modified for EFT start
        CMB%EFTwbds	     = 1.0/(Exp(Params(49)))
	CMB%EFTwbd	     = 1.0/(Exp(Params(50)))
	CMB%EFTGeffG	     = Params(51)
        CMB%omvh2	     = Params(52)
        if (Cosmosettings%EFTFlag .eq. 2 .and. CosmoSettings%DesignerEFTmodel .eq. 4) then
        CMB%EFTphi_0 = (CMB%EFTGeffG*(2.0*CMB%EFTwbds + 3.0)/(2.0*CMB%EFTwbds + 4.0))**(-1.0)
	end if
	if (Cosmosettings%EFTFlag .eq. 2 .and. CosmoSettings%DesignerEFTmodel .eq. 5) then
	CMB%EFTphi_0 = (CMB%EFTGeffG*(2.0*CMB%EFTwbd + 3.0)/(2.0*CMB%EFTwbd + 4.0))**(-1.0)
	end if
	!Modified for EFT end
        ! EFTCOSMOMC MOD END.
