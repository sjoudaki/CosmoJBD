    ...
    procedure :: GetTransfer => CAMBCalc_GetTransfer
    ...
    !Modified for EFT start
    procedure :: CMBH0BD => CAMBCalc_getH0frombackground
    !Modified for EFT end
    ...
    !MODIFIED for EFT start
    P%EFTwbd = CMB%EFTwbd
    P%EFTwbds = CMB%EFTwbds
    P%EFTphi_0 = CMB%EFTphi_0
    !MODIFIED for EFT end
...
!        end if
    ...
    if(CosmoSettings%use_growth_kz) then
        call this%GetTransfer(M,PK,transfer_growth)
    ...
    call this%CMBToCAMB(CMB, P)
    !Modified for EFT start
    call CAMBParams_Set(P,Haux,phi_aux,error,WantReion)
    !Modified for EFT end
    ...
    call this%CMBToCAMB(CMB, P)
    !Modified for EFT start
    call CAMBParams_Set(P,Haux,phi_aux,error)
    !Modified for EFT end
    if (error/= 0) then
        GetOpticalDepth = -1
    else
    ...
    !Modified for EFT start
    function CAMBCalc_getH0frombackground(this) result(H0_bd)
    use ModelParams
    class(CAMB_Calculator) :: this
    real(mcp) H0_bd

    H0_bd = get_H0_from_background()

    end function CAMBCalc_getH0frombackground
    !Modified for EFT end
    ...
    !MODIFIED for EFT start
    P%EFTpot = CosmoSettings%EFTpot
    P%EFTbdback_debug = CosmoSettings%EFTbdback_debug
    !MODIFIED for EFT end
