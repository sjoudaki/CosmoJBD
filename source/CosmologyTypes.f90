...
!        real(mcp) :: z_outputs(1) = [0.57_mcp]
	...
	!Modified for EFT end
        ...
        real(mcp) h, H0, tau
        real(mcp) w, wa
        real(mcp) baryfeed, barybloat !meadfit, SJ
	...
	!Modified for EFT start
	real(mcp) :: EFTwbds, EFTwbd, EFTGeffG, EFTphi_0
	!Modified for EFT end
        ! EFTCOSMOMC MOD END.
    ...
    !Modified for EFT start
    this%EFTpot = Ini%Read_Int('EFTpot',0)
    this%EFTbdback_debug = Ini%Read_Int('EFTbdback_debug',0)
    !Modified for EFT end
